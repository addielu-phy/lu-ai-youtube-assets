...
- GitHub Pages：storyboard 版已同步到穩定手機索引 `https://addielu-phy.github.io/lu-ai-youtube-assets/mobile-index/index.html`（內容 commit `9f49f43`；README／壓縮包 cleanup commit `b7f0971`），本輪 slide-video 草稿已同步到穩定手機索引（內容 commit `8f50930`；cleanup commit `ad2a385`）；長片 upload kit／正式講義已同步到穩定 GitHub Pages（commit `7ebf311`）；社群宣傳卡與更新後輕量包已同步到穩定 GitHub Pages（commit `d61c89e`，cache-busted 驗證 index／PNG／upload kit／archive 皆 200）。
