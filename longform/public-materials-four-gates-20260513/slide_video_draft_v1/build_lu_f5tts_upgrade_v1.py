...
SLIDES = [
    BASE/'slides/01_hook_four_gates.png',
    BASE/'slides/02_source_gate.png',
    BASE/'slides/03_license_gate.png',
    BASE/'slides/04_privacy_ai_label_gate.png',
    BASE/'slides/05_classroom_flow.png',
]
...
new_mp4 = OUT / 'public-materials-four-gates-slide-video-draft-lu-f5tts-v1.mp4'
...
draw.text((50,28),'公開教材四關｜Lu F5-TTS v1 QA contact sheet',font=title_font,fill=(32,45,62))
...
manifest.update({
    ...
    'next_auto_push': 'Continue voice remediation queue with shorts/image-license-check-before-slide-20260513, or upload Lu voice versions after YouTube/Google login.'
})
...
archive = OUT/'public-materials-four-gates-lu-f5tts-v1-kit-20260517.tar.gz'
members = [
    ...
    new_mp4, OUT/'public-materials-four-gates-slide-video-draft-v1.mp4', OUT/'youtube-upload-kit-longform-v1.md', OUT/'public-materials-four-gates-shareable-handout-v1.md', qa,
...
with tarfile.open(archive,'r:gz') as tar:
    names = tar.getnames()
print(json.dumps({'new_mp4': str(new_mp4), 'duration': duration(new_mp4), 'audio_duration': duration(fit), 'archive': str(archive), 'archive_members': len(names), 'qa': str(qa)}, ensure_ascii=False, indent=2))
