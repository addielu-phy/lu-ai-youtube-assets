...
PKG = Path('/home/adl/youtube-lu-ai-channel/shorts/public-before-share-check-20260513')
...
chunks = [
    ...
    '第三，AI 標註：哪裡由 AI 協助，哪裡由學生判斷，要說清楚。最後一句最重要：AI 可以協助檢查，但公開責任由老師把關。',
]
...
old_mp4 = PKG / 'public-before-share-check-shorts-35s.mp4'
new_mp4 = PKG / 'public-before-share-check-shorts-35s-lu-f5tts-v1.mp4'
...
segments = [
    (0.0, 5.0, '能不能公開，不是看作品漂不漂亮。'),
    (5.0, 9.0, '學生作品放上網前，請 AI 先做三件事。'),
    (9.0, 16.5, '第一，匿名化：姓名、臉、班級、學號，先拿掉。'),
    (16.5, 24.5, '第二，授權來源：圖片、音樂、資料，不是 AI 找到就能公開。'),
    (24.5, 35.0, '第三，AI 標註：協助與判斷要分清楚，老師最後把關。'),
]
...
imgs = [Image.open(PKG/'cover_public_before_share_check_v1.png').convert('RGB')] + [Image.open(CHECKS/n).convert('RGB') for n in frame_names]
...
manifest.update({
    'status': '35s Shorts MP4/upload kit/public checklist complete; Lu Teacher F5-TTS v1 voice upgrade ready',
...
archive = PKG / 'public-before-share-check-lu-f5tts-v1-kit-20260517.tar.gz'
members = ['README.md','manifest.json','build_lu_f5tts_upgrade_v1.py','youtube-upload-kit.md','student-work-public-checklist-v1.md',new_mp4.name,'public-before-share-check-shorts-35s.mp4','cover_public_before_share_check_v1.png','narration_35s.txt',mp3.name,fit.name,'narration_35s_lu_f5tts_v1.vtt','narration_35s_lu_f5tts_v1.srt','narration_35s_edge_hsiaoyu.mp3','narration_35s_edge_hsiaoyu.vtt','narration_35s_edge_hsiaoyu.srt','checks/qa_lu_f5v1_sheet.png','checks/frame_early_lu_f5v1.png','checks/frame_mid_lu_f5v1.png','checks/frame_late_lu_f5v1.png','checks/qa_sheet_mp4.png']
with tarfile.open(archive, 'w:gz') as tar:
    for rel in members:
        p = PKG / rel
        if p.exists():
            tar.add(p, arcname=f'public-before-share-check-20260513/{rel}')
