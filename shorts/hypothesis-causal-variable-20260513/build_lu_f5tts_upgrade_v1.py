...
PKG = Path('/home/adl/youtube-lu-ai-channel/shorts/hypothesis-causal-variable-20260513')
...
chunks = [
    '讓 AI 幫學生寫假設前，先加一句話。',
    '如果我改變什麼，那麼什麼會怎樣，因為什麼。',
    '例如，不要只寫摩擦力會變小。',
    '要寫，如果斜面角度變大，那麼小車到底端速度會增加，因為重力沿斜面的分量變大。',
    '這句話會同時鎖住操縱變因、應變變因和原因。沒有變因到結果到原因，就先退件，讓 AI 重寫。',
]
...
old_mp4 = PKG / 'hypothesis-causal-variable-shorts-35s.mp4'
new_mp4 = PKG / 'hypothesis-causal-variable-shorts-35s-lu-f5tts-v1.mp4'
...
segments = [
    (0.0, 5.0, '讓 AI 幫學生寫假設前，先加一句話。'),
    (5.0, 10.5, '如果我改變什麼，那麼什麼會怎樣，因為什麼。'),
    (10.5, 15.5, '不要只寫摩擦力會變小。'),
    (15.5, 26.5, '改成：如果斜面角度變大，小車到底端速度會增加，因為重力沿斜面的分量變大。'),
    (26.5, 35.0, '它鎖住變因、結果和原因；缺一個，就退件重寫。'),
]
...
imgs = [Image.open(PKG/'cover_hypothesis_causal_variable_v1.png').convert('RGB')] + [Image.open(CHECKS/n).convert('RGB') for n in frame_names]
...
archive = PKG / 'hypothesis-causal-variable-lu-f5tts-v1-kit-20260517.tar.gz'
members = ['README.md','manifest.json','build_lu_f5tts_upgrade_v1.py','youtube-upload-kit.md',new_mp4.name,'hypothesis-causal-variable-shorts-35s.mp4','cover_hypothesis_causal_variable_v1.png','narration_35s.txt',mp3.name,fit.name,'narration_35s_lu_f5tts_v1.vtt','narration_35s_lu_f5tts_v1.srt','narration_35s_edge_hsiaoyu.mp3','narration_35s_edge_hsiaoyu.vtt','narration_35s_edge_hsiaoyu.srt','checks/qa_lu_f5v1_sheet.png','checks/frame_early_lu_f5v1.png','checks/frame_mid_lu_f5v1.png','checks/frame_late_lu_f5v1.png','checks/qa_sheet_mp4.png']
with tarfile.open(archive, 'w:gz') as tar:
    for rel in members:
        p = PKG / rel
        if p.exists():
            tar.add(p, arcname=f'hypothesis-causal-variable-20260513/{rel}')
