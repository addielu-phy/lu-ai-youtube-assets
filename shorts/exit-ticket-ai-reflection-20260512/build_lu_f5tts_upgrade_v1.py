...
PKG = Path('/home/adl/youtube-lu-ai-channel/shorts/exit-ticket-ai-reflection-20260512')
...
old_mp4 = PKG / 'exit-ticket-ai-reflection-shorts-35s.mp4'
new_mp4 = PKG / 'exit-ticket-ai-reflection-shorts-35s-lu-f5tts-v1.mp4'
...
segments = [
    (0.0, 5.0, '下課前三分鐘，不要只問 AI 好不好用。'),
    (5.0, 10.5, '請學生寫一句：我剛才怎麼判斷。'),
    (10.5, 16.0, '我原本預期會看到什麼？'),
    (16.0, 23.5, 'AI 哪裡有證據，哪裡只是說得順？'),
    (23.5, 35.0, '下次先檢查哪一步？讓 AI 使用留下學習證據。'),
]
...
imgs = [Image.open(PKG/'cover_exit_ticket_ai_reflection_v1.png').convert('RGB')] + [Image.open(CHECKS/n).convert('RGB') for n in frame_names]
...
manifest.update({
    'status': '35s Shorts MP4/upload kit/teacher printable complete; Lu Teacher F5-TTS v1 voice upgrade ready',
...
archive = PKG / 'exit-ticket-ai-reflection-lu-f5tts-v1-kit-20260518.tar.gz'
members = ['README.md','manifest.json','build_lu_f5tts_upgrade_v1.py','youtube-upload-kit.md','exit-ticket-ai-reflection-teacher-printable-v1.md',new_mp4.name,'exit-ticket-ai-reflection-shorts-35s.mp4','cover_exit_ticket_ai_reflection_v1.png','narration_35s.txt',mp3.name,fit.name,'narration_35s_lu_f5tts_v1.vtt','narration_35s_lu_f5tts_v1.srt','narration_35s_edge_hsiaoyu.mp3','narration_35s_edge_hsiaoyu.vtt','narration_35s_edge_hsiaoyu.srt','checks/qa_lu_f5v1_sheet.png','checks/frame_early_lu_f5v1.png','checks/frame_mid_lu_f5v1.png','checks/frame_late_lu_f5v1.png','checks/qa_cover_frames_sheet.png']
with tarfile.open(archive, 'w:gz') as tar:
    for rel in members:
        p = PKG / rel
        if p.exists():
            tar.add(p, arcname=f'exit-ticket-ai-reflection-20260512/{rel}')
    for p in sorted((PKG/'slides').glob('slide_*.png')):
        tar.add(p, arcname=f'exit-ticket-ai-reflection-20260512/slides/{p.name}')
