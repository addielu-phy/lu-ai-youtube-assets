...
chunks = [
    'AI 幫你整理課堂摘要，最危險的是寫得太像真的。',
    '先刪可辨識學生的內容：座號、特殊事件，或可回推的錯誤。',
    '再刪貼標籤的評語：懶散、粗心、程度差。',
    '也刪沒有證據的全班評價，像某組最差、全班都不懂。',
...
for i, t in enumerate(chunks, 1):
    ...
    cmd = [str(F5), '--model', 'F5TTS_v1_Base', '--ref_audio', str(REF_AUDIO), '--ref_text', REF_TEXT,
           ...
           '--device', 'cuda', '--speed', '1.10', '--remove_silence', '--no_legacy_text']
...
fit = PKG / 'narration_35s_lu_f5tts_v2.wav'
mp3 = PKG / 'narration_35s_lu_f5tts_v2.mp3'
...
new_mp4 = PKG / 'class-summary-redaction-review-shorts-35s-lu-f5tts-v2.mp4'
...
(PKG/'narration_35s_lu_f5tts_v2.vtt').write_text('WEBVTT\n\n'+'\n\n'.join(f'{ts(a)} --> {ts(b)}\n{text}' for a,b,text in segments)+'\n', encoding='utf-8')
(PKG/'narration_35s_lu_f5tts_v2.srt').write_text('\n\n'.join(f'{i}\n{ts(a, True)} --> {ts(b, True)}\n{text}' for i,(a,b,text) in enumerate(segments,1))+'\n', encoding='utf-8')

for name, sec in [('frame_early_lu_f5v2.png','2.0'),('frame_mid_lu_f5v2.png','17.0'),('frame_late_lu_f5v2.png','32.0')]:
...
manifest.update({
    ...
    'audio_source': 'Lu Teacher F5-TTS voice clone v2',
    'narration_lu_voice': mp3.name,
    'subtitles_lu_voice': ['narration_35s_lu_f5tts_v2.vtt','narration_35s_lu_f5tts_v2.srt'],
    'voice_upgrade_note': 'Original package used Edge zh-TW-HsiaoYuNeural; v2 replaces audio with Lu Teacher F5-TTS voice clone, chunked generation, loudnorm, fitted to 35s.'
})
...
'''
if '## 音訊升級 v2（Lu Teacher F5-TTS）' not in readme:
    (PKG/'README.md').write_text(readme + append, encoding='utf-8')

archive = PKG / 'class-summary-redaction-review-lu-f5tts-v2-kit-20260515.tar.gz'
members = ['README.md','manifest.json','build_lu_f5tts_upgrade_v2.py','youtube-upload-kit.md','summary-redaction-teacher-checklist-v1.md',new_mp4.name,'class-summary-redaction-review-shorts-35s.mp4','cover_class_summary_redaction_review_v1.png','narration_35s.txt',mp3.name,fit.name,'narration_35s_lu_f5tts_v2.vtt','narration_35s_lu_f5tts_v2.srt','narration_35s_edge_hsiaoyu.mp3','narration_35s_edge_hsiaoyu.vtt','narration_35s_edge_hsiaoyu.srt','class_summary_redaction_review.ffconcat','checks/qa_cover_frames_sheet.png','checks/frame_early_lu_f5v2.png','checks/frame_mid_lu_f5v2.png','checks/frame_late_lu_f5v2.png']
...
for dest in [PAGES, ASSETS_PAGES]:
    ...
    for rel in [new_mp4.name, mp3.name, fit.name, 'narration_35s_lu_f5tts_v2.vtt','narration_35s_lu_f5tts_v2.srt','README.md','manifest.json','youtube-upload-kit.md','build_lu_f5tts_upgrade_v2.py',archive.name]:
        src = PKG / rel
        if src.exists():
            (dest/rel).parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(src, dest/rel)
    (dest/'checks').mkdir(exist_ok=True)
    for rel in ['frame_early_lu_f5v2.png','frame_mid_lu_f5v2.png','frame_late_lu_f5v2.png']:
        src = PKG/'checks'/rel
