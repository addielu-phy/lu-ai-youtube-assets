...
PKG = Path('/home/adl/youtube-lu-ai-channel/shorts/known-unknown-boundary-map-20260514')
PAGES_ROOTS = [
    Path('/home/adl/pages/lu-ai-youtube-assets/shorts/known-unknown-boundary-map-20260514'),
    Path('/home/adl/lu-ai-youtube-assets-pages/shorts/known-unknown-boundary-map-20260514'),
]
...
chunks = [
    '學生問 AI 之前，先別急著把題目丟進去。',
    '先畫一張我知道、我不知道邊界圖：左邊寫已知條件，右邊寫還缺什麼。',
...
old_mp4 = PKG / 'known-unknown-boundary-map-shorts-35s.mp4'
new_mp4 = PKG / 'known-unknown-boundary-map-shorts-35s-lu-f5tts-v1.mp4'
...
segments = [
    (0.0, 5.8, '學生問 AI 前，先別急著把題目丟進去。'),
    (5.8, 13.4, '先畫「我知道／我不知道」邊界圖。'),
    (13.4, 22.0, '斜面題先列角度、質量、加速度，也標出摩擦和方向缺口。'),
    (22.0, 29.2, '請 AI 先檢查邊界圖，指出缺口，再給解法。'),
    (29.2, 35.0, '學生先說清楚自己負責的部分。'),
]
...
# simple QA contact sheet for Lu version
imgs = [Image.open(PKG/'cover_known_unknown_boundary_map_v1.png').convert('RGB')] + [Image.open(CHECKS/n).convert('RGB') for n in frame_names]
...
archive = PKG / 'known-unknown-boundary-map-lu-f5tts-v1-kit-20260516.tar.gz'
members = ['README.md','manifest.json','build_lu_f5tts_upgrade_v1.py','youtube-upload-kit.md','known-unknown-boundary-map-student-worksheet-v1.md',new_mp4.name,'known-unknown-boundary-map-shorts-35s.mp4','cover_known_unknown_boundary_map_v1.png','narration_35s.txt',mp3.name,fit.name,'narration_35s_lu_f5tts_v1.vtt','narration_35s_lu_f5tts_v1.srt','narration_35s_edge_hsiaoyu.mp3','narration_35s_edge_hsiaoyu.vtt','narration_35s_edge_hsiaoyu.srt','checks/qa_lu_f5v1_sheet.png','checks/frame_early_lu_f5v1.png','checks/frame_mid_lu_f5v1.png','checks/frame_late_lu_f5v1.png','checks/qa_cover_frames_sheet.png']
with tarfile.open(archive, 'w:gz') as tar:
    for rel in members:
        p = PKG / rel
        if p.exists():
            tar.add(p, arcname=f'known-unknown-boundary-map-20260514/{rel}')
...
for dest in PAGES_ROOTS:
    ...
    for rel in ['frame_early_lu_f5v1.png','frame_mid_lu_f5v1.png','frame_late_lu_f5v1.png','qa_lu_f5v1_sheet.png']:
        src = CHECKS / rel
