...
PKG = Path('/home/adl/lu-ai-youtube-assets/shorts/homework-feedback-teacher-review-20260512')
...
for i, text in enumerate(chunks, 1):
    ...
    cmd = [str(F5), '--model', 'F5TTS_v1_Base', '--ref_audio', str(REF_AUDIO), '--ref_text', REF_TEXT,
           ...
           '--device', 'cuda', '--speed', '1.06', '--remove_silence', '--no_legacy_text']
...
old_mp4 = PKG / 'homework-feedback-teacher-review-shorts-35s.mp4'
new_mp4 = PKG / 'homework-feedback-teacher-review-shorts-35s-lu-f5tts-v1.mp4'
...
imgs = [Image.open(PKG/'cover_homework_feedback_teacher_review_v1.png').convert('RGB')] + [Image.open(CHECKS/n).convert('RGB') for n in frame_names]
...
upload_path = PKG/'youtube-upload-kit.md'
...
if new_mp4.name not in upload:
    ...
    upload_path.write_text(upload.rstrip() + insert + '\n', encoding='utf-8')
archive = PKG / 'homework-feedback-teacher-review-lu-f5tts-v1-kit-20260525.tar.gz'
members = ['README.md','manifest.json','build_lu_f5tts_upgrade_v1.py','youtube-upload-kit.md','ai-feedback-review-teacher-checklist-v1.md',new_mp4.name,'homework-feedback-teacher-review-shorts-35s.mp4','cover_homework_feedback_teacher_review_v1.png','narration_35s.txt',mp3.name,fit.name,'narration_35s_lu_f5tts_v1.vtt','narration_35s_lu_f5tts_v1.srt','narration_35s_edge_hsiaoyu.mp3','narration_35s_edge_hsiaoyu.vtt','narration_35s_edge_hsiaoyu.srt','checks/qa_lu_f5v1_sheet.png','checks/frame_early_lu_f5v1.png','checks/frame_mid_lu_f5v1.png','checks/frame_late_lu_f5v1.png','checks/qa_cover_frames_sheet.png']
with tarfile.open(archive, 'w:gz') as tar:
    for rel in members:
        p = PKG / rel
        if p.exists():
            tar.add(p, arcname=f'homework-feedback-teacher-review-20260512/{rel}')
    for p in sorted((PKG/'slides').glob('slide_*.png')):
        tar.add(p, arcname=f'homework-feedback-teacher-review-20260512/slides/{p.name}')
